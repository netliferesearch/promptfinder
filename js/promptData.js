...
async function hasOffscreenDocument(path) {
    if (typeof chrome === 'undefined' || !chrome.runtime || !chrome.runtime.getManifest().offscreen) {
        console.warn("Offscreen permission or document path not declared in manifest, or chrome.runtime not available.");
        return false;
    }
...
export const signInWithGoogle = async () => {
    ...
    try {
        ...
        if (response && response.success) {
            ...
            const errMsg = response?.error?.message || "An unknown error occurred during Google Sign-In via offscreen.";
        ...
        Utils.handleError(`Google Sign-In client-side error (v9): ${error.message}`, { userVisible: true, originalError: error });
        return Promise.reject(error);
    }
};
...
export const addPrompt = async (promptData) => {
    ...
    try {
        ...
        Utils.handleError(`Error adding prompt to Firestore (v9): ${error.message}`, { userVisible: true, originalError: error });
        return null;
    }
};
...
const formatLoadedPrompt = (docSnapshot) => {
    ...
    const convertTimestamp = (ts) => ts instanceof Timestamp ? ts.toDate().toISOString() : (ts ? new Date(ts).toISOString() : null);
    return {
        id: docSnapshot.id,
        ...data,
...
export const loadPrompts = async () => {
    ...
    try {
        if (currentUser) {
            const userPromptsQuery = query(collection(db, 'prompts'), where('userId', '==', currentUser.uid));
            ...
            const publicPromptsQuery = query(collection(db, 'prompts'), where('isPrivate', '==', false), where('userId', '!=', currentUser.uid));
        ...
        Utils.handleError(`Error loading prompts from Firestore (v9): ${error.message}`, { userVisible: true, originalError: error });
        return [];
    }
};
    ...
    try {
        ...
        if (handleError && Utils && Utils.handleError) {
            if (!(error.message.includes("not found in Firestore") && throwIfNotFound)) {
                Utils.handleError(`Error retrieving prompt ${promptId} (v9): ${error.message}`, { userVisible: true, originalError: error });
            }
        }
...
export const updatePrompt = async (promptId, updates) => {
    ...
    try {
        ...
        if (!docSnap.exists()) {
            Utils.handleError(`Prompt with ID ${promptId} not found for update (v9).`, { userVisible: true });
            return null;
        }
        if (docSnap.data().userId !== currentUser.uid) {
            Utils.handleError("You do not have permission to update this prompt (v9).", { userVisible: true });
            return null;
        }
        ...
        Utils.handleError(`Error updating prompt ${promptId} in Firestore (v9): ${error.message}`, { userVisible: true, originalError: error });
        return null;
    }
};
...
export const deletePrompt = async (promptId) => {
    ...
    try {
        ...
        if (!docSnap.exists()) {
            Utils.handleError(`Prompt with ID ${promptId} not found for deletion (v9).`, { userVisible: true });
            return false;
        }
        if (docSnap.data().userId !== currentUser.uid) {
            Utils.handleError("You do not have permission to delete this prompt (v9).", { userVisible: true });
            return false;
        }
...
export const toggleFavorite = async (promptId) => {
    ...
    try {
        ...
        if (promptData.userId !== currentUser.uid) {
            Utils.handleError("You can only favorite/unfavorite your own prompts directly (v9).", { userVisible: true });
            return null;
        }
        ...
        Utils.handleError(`Error toggling favorite for prompt ${promptId} (v9): ${error.message}`, { userVisible: true, originalError: error });
        return null;
    }
};
...
export const updatePromptRating = async (promptId, rating) => {
    ...
    try {
        ...
        Utils.handleError(`Error updating userRating for prompt ${promptId} (v9): ${error.message}`, { userVisible: true, originalError: error });
        return null;
    }
};
...
export const copyPromptToClipboard = async promptId => {
    try {
        ...
        Utils.handleError(`Error copying to clipboard (v9): ${error.message}`, { userVisible: true, originalError: error });
        return false;
    }
};
